...
You are working on **NextSQL**, a native multimodel database engine with its own protocol, SQL dialect, storage engine, optimizer, security model, drivers, operational tooling, and Studio.
...
# 18. P30 — removed from product

NextSQL Intelligence, built-in RAG, RAG Playground, and `CREATE RETRIEVER`
are **not in the product**. Do not implement them. Studio has no AI
assistant. Full-text, vector, and hybrid search remain ordinary SQL
