...
| `system.views` | `view_name, owner, columns, definition` | One row per view, ordered by name. `definition` is the stored defining query. Shown to an admin or to the view's owner only: a definition names the tables it reads. |
