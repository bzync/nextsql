...
Published packages, checksums, and a feature comparison are on [Downloads](/download). Direct files: [GitHub release v0.0.3](https://github.com/bzync/nextsql/releases/tag/v0.0.3). For a container, see [Docker](/docs/docker). For the GUI, see [Admin](/docs/admin).
...
Verify the file against [SHA256SUMS](https://github.com/bzync/nextsql/releases/download/v0.0.3/SHA256SUMS) before you run it.

### Debian / Ubuntu (`.deb`)

```bash
curl -fsSL -O https://github.com/bzync/nextsql/releases/download/v0.0.3/nextsql_0.0.3_amd64.deb
curl -fsSL -O https://github.com/bzync/nextsql/releases/download/v0.0.3/SHA256SUMS
sha256sum -c --ignore-missing SHA256SUMS
sudo dpkg -i nextsql_0.0.3_amd64.deb
```

### Self-extracting installer (`.run`)

```bash
curl -fsSL -O https://github.com/bzync/nextsql/releases/download/v0.0.3/nextsql-0.0.3-linux-amd64.run
curl -fsSL -O https://github.com/bzync/nextsql/releases/download/v0.0.3/SHA256SUMS
sha256sum -c --ignore-missing SHA256SUMS
chmod +x nextsql-0.0.3-linux-amd64.run
sudo ./nextsql-0.0.3-linux-amd64.run
```

### Portable tarball

```bash
curl -fsSL -O https://github.com/bzync/nextsql/releases/download/v0.0.3/nextsql-0.0.3-linux-amd64.tar.gz
tar -xzf nextsql-0.0.3-linux-amd64.tar.gz
sudo ./nextsql-0.0.3-linux-amd64/install.sh
```
...
```bash
docker pull bzynchub/nextsql:0.0.3
```
...
```bash
go install github.com/bzync/nextsql/cmd/nextsql@v0.0.3
go install github.com/bzync/nextsql/cmd/nextsqld@v0.0.3
go install github.com/bzync/nextsql/cmd/nextsql-bench@v0.0.3
go install github.com/bzync/nextsql/cmd/nextsql-auth-broker@v0.0.3
go install github.com/bzync/nextsql/cmd/nextsql-admin@v0.0.3
```
...
Official drivers are in the repo tree under `drivers/`. Their package versions
are independent of the engine version.

| Runtime | Install |
|---|---|
| Go | `go get github.com/bzync/nextsql/drivers/go@v0.0.3` |
