...
This site documents **0.0.5**, the current release. A live install uses `nextsql setup --profile production`. NextSQL Admin Setup, Operations, and Studio are available on loopback. Treat the release as an engine under measurement until you have run `nextsql-bench --slo` and the crash/HA suites on your hardware. Hard limits and unimplemented SQL are listed under [Limits](/docs/limits).
