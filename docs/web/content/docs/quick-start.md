...
Install `nextsql` and `nextsqld` first — Linux packages are on [Downloads](/download) (on Windows, use them inside [WSL 2](/docs/install#windows-wsl-2)); steps are in [Install](/docs/install). For a container, see [Docker](/docs/docker). `nextsql setup --profile production` is the non-interactive live-install path (fail-closed preflight plus operational defaults). This walkthrough uses `nextsql init` so you can see each file. Use two terminals. Paths below are examples. In production put the key file on a different volume from `--data-dir`.
