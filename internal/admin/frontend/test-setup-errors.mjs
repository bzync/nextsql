...
try {
  ...
  const cases = [
    ...
    ["nextsql: nextsql already_exists: nextsql setup: recovery key file /etc/nextsql/recovery.key already exists; setup never overwrites key material", /recovery key file already exists/i],
    ["nextsql invalid_argument: setup.Params: recoveryKeyOut cannot be used with skipInit: no database is initialized to create a recovery key for", /skipping database creation/i],
    ["nextsql invalid_argument: setup.Params: instanceRecoveryKeyOut requires recoveryKeyOut", /configured together/i],
