...
// NextSQL RBAC. The broker intersects the policy-mapped roles with this set so
// an external identity can only ever narrow a real grant (design invariant I1).
// It is optional: when nil the broker mints the policy-mapped roles as-is and
// relies on the server's `ACL.AllowedScoped` to drop any role the principal
// does not hold. Wiring a real membership feed is a later P25 increment.
type RoleMembershipFunc func(principal string) ([]string, error)
...
func (b *Broker) load() error {
	...
	for _, p := range b.cfg.Profiles {
		...
		verifiers[p.Name] = &profileVerifier{profile: p, idtoken: iv}
	}
	b.mu.Lock()
	b.policy, b.keyset, b.verifiers = policy, keyset, verifiers
	b.mu.Unlock()
	return nil
}
...
// heldRoles returns the principal's RBAC roles, or nil when no feed is wired.
func (b *Broker) heldRoles(principal string) ([]string, bool, error) {
	if b.members == nil {
		return nil, false, nil
	}
	roles, err := b.members(strings.ToLower(strings.TrimSpace(principal)))
	if err != nil {
		return nil, true, err
	}
