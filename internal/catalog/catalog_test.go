...
func TestTableEncodeRoundTrip(t *testing.T) {
	...
	trailer := len(got.Indexes)*17 + 1 + len(got.Columns)*5 // v10 flag, v11 ENUM count, v12 collection flag, v13 encryption mode
...
func TestTableEncodeFulltextAnalyzerV9(t *testing.T) {
	...
	v8 := append([]byte(nil), raw[:len(raw)-3-len(tab.Columns)*5]...)
	...
	bad[len(bad)-len(tab.Columns)*5-3] = 99
