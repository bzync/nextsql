...
// TestJSONPathArrayIndex pins docs/json.md's own array-index example
// (`tags.0` — "A numeric part indexes an array") end to end. This never
// actually parsed before internal/sql/parser's pathIndexPart: the lexer
// fuses a dot immediately followed by a digit into one leading-dot Number
// literal (the same rule that lexes ".5" as the float 0.5), so `tags.0`
// arrived at the parser's Path-continuation loop as Ident("tags") followed
// by Number(".0"), never a separate Dot then Number(0) — confirmed live
// against a real nextsqld before this fix (a manual repro, not assumed),
// where the exact statements below failed with "unexpected token after
// statement" and `metadata.tags."0"` (a quoted numeric segment) was the
// only way to reach the same value.
func TestJSONPathArrayIndex(t *testing.T) {
	...
	execOK(t, s, `INSERT INTO products (name, metadata) VALUES
		('alpha', '{"tags":["red","blue","green"],"items":[{"sku":"a1"},{"sku":"a2"}]}'),
		('beta', '{"tags":["blue"],"items":[{"sku":"b1"}]}')`)

	got := execOK(t, s, `SELECT metadata.tags.0, metadata.tags.1, metadata.tags.2 FROM products WHERE name = 'alpha'`)
	if len(got.Rows) != 1 || got.Rows[0][0].Str != "red" || got.Rows[0][1].Str != "blue" || got.Rows[0][2].Str != "green" {
		t.Fatalf("array-index extract %+v", got.Rows)
	}

	// Nested: an array of objects, two path segments deep past the index.
	got = execOK(t, s, `SELECT metadata.items.0.sku, metadata.items.1.sku FROM products WHERE name = 'alpha'`)
	if len(got.Rows) != 1 || got.Rows[0][0].Str != "a1" || got.Rows[0][1].Str != "a2" {
		t.Fatalf("nested array-index extract %+v", got.Rows)
	}

	// An out-of-range index is a missing path, i.e. SQL NULL, not an error.
	got = execOK(t, s, `SELECT metadata.tags.5 FROM products WHERE name = 'beta'`)
	if len(got.Rows) != 1 || !got.Rows[0][0].Null {
		t.Fatalf("out-of-range index %+v", got.Rows)
	}

	got = execOK(t, s, `SELECT name FROM products WHERE metadata.tags.0 = 'blue'`)
	if len(got.Rows) != 1 || got.Rows[0][0].Str != "beta" {
		t.Fatalf("where array-index path %+v", got.Rows)
	}

	execOK(t, s, `CREATE INDEX tag0_index ON products (metadata.tags.0)`)
	plan := execOK(t, s, `EXPLAIN SELECT name FROM products WHERE metadata.tags.0 = 'blue'`)
	if !explainHas(plan, "IndexScan") || !explainHas(plan, "tag0_index") {
		t.Fatalf("array-index path index plan: %+v", explainOps(plan))
	}
	got = execOK(t, s, `SELECT name FROM products WHERE metadata.tags.0 = 'blue'`)
	if len(got.Rows) != 1 || got.Rows[0][0].Str != "beta" {
		t.Fatalf("indexed array-index path %+v", got.Rows)
	}

	upd := execOK(t, s, `UPDATE products SET metadata = '{"tags":["black"]}' WHERE name = 'alpha'`)
	if upd.Affected != 1 {
		t.Fatalf("updated %d", upd.Affected)
	}
	got = execOK(t, s, `SELECT name FROM products WHERE metadata.tags.0 = 'black'`)
	if len(got.Rows) != 1 || got.Rows[0][0].Str != "alpha" {
		t.Fatalf("updated array-index path %+v", got.Rows)
	}

	// A genuine leading-dot float literal elsewhere is unaffected by the
	// path-parsing fix above.
	got = execOK(t, s, `SELECT 1 + .5 AS n FROM products WHERE name = 'alpha'`)
	if len(got.Rows) != 1 || got.Rows[0][0].Dec.String() != "1.5" {
		t.Fatalf("leading-dot float literal regressed: %+v", got.Rows)
	}
}
...
func TestFulltextHighlight(t *testing.T) {
	...
	execOK(t, s, `CREATE FULLTEXT INDEX ix_body ON articles (body)`)
	execOK(t, s, `CREATE FULLTEXT INDEX ix_title_body ON articles (title, body)`)

	// Exact statement shape generated by Studio's Full-text Explorer: quoted
	// identifiers, primary-key context, one aliased SNIPPET per indexed field,
	// and the native SEARCH order/limit. This proves the frontend helper is
	// grounded in the binder/executor contract, not only the parser grammar.
	got := execOK(t, s, `SELECT "id", SNIPPET("title") AS "title_snippet", SNIPPET("body") AS "body_snippet" FROM "articles" SEARCH "title", "body" FOR 'cat' LIMIT 20`)
	if len(got.Columns) != 3 || got.Columns[1] != "title_snippet" || got.Columns[2] != "body_snippet" || len(got.Rows) < 2 {
		t.Fatalf("Studio full-text result columns=%v rows=%d", got.Columns, len(got.Rows))
	}
	for _, row := range got.Rows {
		if !strings.Contains(row[2].Str, "<mark>") {
			t.Fatalf("Studio full-text snippet missing literal match markers: %q", row[2].Str)
		}
	}
