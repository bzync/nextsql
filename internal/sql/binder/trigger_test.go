...
func TestBindTriggerLifecycleAndDependencies(t *testing.T) {
	...
	bound, err := BindAutomation(stmt, tables, workflows, workflowList, triggers, triggerList, func(string) (*catalog.Schedule, bool) { return nil, false }, nil, func(string) (*catalog.ResourceGroup, bool) { return nil, false }, 3, "alice")
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, sql := range []string{`DROP TABLE orders`, `ALTER TABLE orders ADD COLUMN note STRING`, `DROP WORKFLOW record`, `ALTER WORKFLOW record RENAME TO record2`} {
		...
		if _, err := BindAutomation(candidate, tables, workflows, workflowList, triggers, triggerList, func(string) (*catalog.Schedule, bool) { return nil, false }, nil, func(string) (*catalog.ResourceGroup, bool) { return nil, false }, 4, "alice"); err == nil {
			t.Fatalf("expected trigger dependency rejection: %s", sql)
		}
	}
	rename, _ := parser.Parse(`ALTER TRIGGER audit RENAME TO audit2`)
	if _, err := BindAutomation(rename, tables, workflows, workflowList, triggers, triggerList, func(string) (*catalog.Schedule, bool) { return nil, false }, nil, func(string) (*catalog.ResourceGroup, bool) { return nil, false }, 4, "alice"); err != nil {
		t.Fatal(err)
	}
	drop, _ := parser.Parse(`DROP TRIGGER audit`)
	if _, err := BindAutomation(drop, tables, workflows, workflowList, triggers, triggerList, func(string) (*catalog.Schedule, bool) { return nil, false }, nil, func(string) (*catalog.ResourceGroup, bool) { return nil, false }, 4, "alice"); err != nil {
		t.Fatal(err)
	}
}
...
func TestBindTriggerRejectsUnknownColumnAndArity(t *testing.T) {
	...
	for _, sql := range []string{
		...
		if _, err := BindAutomation(stmt, tables, workflows, nil, func(string) (*catalog.Trigger, bool) { return nil, false }, nil, func(string) (*catalog.Schedule, bool) { return nil, false }, nil, func(string) (*catalog.ResourceGroup, bool) { return nil, false }, 3, "alice"); err == nil {
