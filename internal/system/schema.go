...
func Capabilities() [][]types.Value {
	...
	rows := [][]types.Value{
		...
		rowCap("fulltext", "supported", "full-text SEARCH with simple and language analyzers, prefix/fuzzy/typo matching, HIGHLIGHT/SNIPPET, and multi-field indexes", "0.1.0"),
		...
		rowCap("follower_reads", "experimental", "STRONG leader read barrier; opt-in STALE local reads; replica lag/health in system.replica_health; no routing yet", version.String),
