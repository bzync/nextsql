...
func TestPortableProductionPath(t *testing.T) {
	...
	for _, entry := range entries {
		...
		if ext == ".s" || ext == ".asm" {
			t.Errorf("production vector assembly %q requires profiling and policy review", name)
			continue
		}
		...
		for _, spec := range file.Imports {
			...
			if importPath == "unsafe" || importPath == "C" {
				t.Errorf("production vector file %s imports %q; acceleration requires profiling and policy review", name, importPath)
			}
		}
	}
}
